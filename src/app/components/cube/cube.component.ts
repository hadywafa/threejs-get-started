...
export class CubeComponent implements AfterViewInit {
  ...
  private MaterialPhantom: THREE.MeshBasicMaterial = new THREE.MeshBasicMaterial({
    color: 0xff0000,
    opacity: 0.5,
    transparent: true,
  });
  private MaterialPhantomLine: THREE.LineBasicMaterial = new THREE.LineBasicMaterial({
    color: 0xff0000,
    opacity: 0.5,
    transparent: true,
  });
  ...
  createSphereHelper(x: number, y: number, z: number, radius: number) {
    let geometry: THREE.SphereGeometry = new THREE.SphereGeometry(radius);
  ...
  createRoomFunction() {
    if (this.MODE === "_ROOMCREATION" && this.floorPointsArray.length > 2) {
      ...
      let geometry = new THREE.ShapeGeometry(roomtShape);
      ...
      geometryLine.setFromPoints([
        ...
        new THREE.Vector3(
          this.floorPointsArray[this.floorPointsArray.length - 1].x,
          0,
          this.floorPointsArray[this.floorPointsArray.length - 1].y
        ),
      ]);
      ...
      for (let index = 0; index < this.linesRoomArray.length; index++) {
        ...
        let lineForCalc = new THREE.Line3(
          this.linesRoomArray[index].geometry.vertices[0].clone(),
          this.linesRoomArray[index].geometry.vertices[1].clone()
        );
        ...
        let wallmaterialPlane: THREE.MeshBasicMaterial = new THREE.MeshBasicMaterial({
        ...
        if (index < this.linesRoomArray.length - 1) {
          wallPlane.rotation.y = angle + THREE.MathUtils.degToRad(180);
        } else {
        ...
        let triangleC = new THREE.Triangle(
          this.linesRoomArray[index].geometry.vertices[0].clone(),
          this.linesRoomArray[index].geometry.vertices[1].clone(),
          this.linesRoomArray[indexForTriangle + 1].geometry.vertices[1].clone()
        );
        ...
        let vertexNormalsHelper = new VertexNormalsHelper(wallPlane, 0.5);
  ...
  drawLine(x: number, y: number, z: number, x1: number, y1: number, z1: number, material: THREE.LineBasicMaterial) {
    ...
    geometry.setFromPoints([new THREE.Vector3(x, y, z), new THREE.Vector3(x1, y1, z1)]);
    ...
    if (intersects.length > 0) {
      ...
      if (this.MODE === "_ROOMCREATION") {
        ...
        if (intersects[0].object.name === "mainPlane") {
          ...
          if (this.floorPointsArray.length > 2) {
            //если точек больше  двух начинаем рисовать многоугольник
          ...
          if (this.floorPointsArray.length > 1) {
            ...
            geometry.setFromPoints([
              new THREE.Vector3(
                this.floorPointsArray[this.floorPointsArray.length - 2].x,
                0,
                this.floorPointsArray[this.floorPointsArray.length - 2].y
              ),
              new THREE.Vector3(
                this.floorPointsArray[this.floorPointsArray.length - 1].x,
                0,
  ...
  onDocumentMouseDown(event: any) {
    ...
    if (intersects.length > 0) {
      ...
      if (this.MODE === "_SIMPLEPARRALILLEPIPED") {
        ...
        if (intersects[0].object.name.includes("Room")) {
          // будем рисовать параллелепипед только в комнате
    ...
    if (intersects.length > 0) {
        ...
        if (this.firstPoint) {
          if (intersects[0].object.name.includes("Room")) {
            ...
            if (this.firstPoint) {
              ...
              this.Phantom = this.createParallelepiped(
                this.firstPoint.x + (this.lastPoint.x - this.firstPoint.x) / 2,
                0.05,
      ...
      if (this.MODE === "_MOVE") {
        ...
        if (this.movement.moving) {
          ...
          if (!this.magnitized) {
            // если параллелепипед не примагничен, то двигаем его
          ...
          for (let vertexIndex = 0; vertexIndex < this.movement.obj.geometry.vertices.length; vertexIndex++) {
            ...
            if (collisionResults.length > 0 && collisionResults[0].object.name === "wall") {
              //находим пересечения его вершин со стеной
          ...
          if (this.magnetMode) {
            ...
            if (collisionResultsLine.length > 0 && collisionResultsLine[0].object.name === "wall") {
              //если он что-то пересек и это еще и стена
  ...
  onDocumentMouseUp(event: MouseEvent) {
    ...
    if (intersects.length > 0) {
      if (this.MODE === "_SIMPLEPARRALILLEPIPED") {
        ...
        if (intersects[0].object.name.includes("Room")) {
          //проверяем, что мышь находится в комнате
          // if (intersects[ 0 ].object.name === "mainPlane"){
          if (this.firstPoint) {
            var intersect = intersects[0];
            this.lastPoint.set(intersect.point.x, intersect.point.z);

            if (this.firstPoint.x - this.lastPoint.x !== 0 || this.firstPoint.y - this.lastPoint.y !== 0) {
              //проверим, что есть сдвиг, иначе он создаст куб размерами по умолчанию 1

              let Parallelepiped: THREE.Mesh;
              let MaterialParallelepiped: THREE.MeshBasicMaterial = new THREE.MeshBasicMaterial({
              ...
              let smallPar = this.createParallelepiped(
                0,
                widthY,
                -height / 2 + height / 6,
                widthX,
  ...
  private createScene() {
    ...
    let geomPlane = new THREE.PlaneGeometry(20, 20, 2, 2);
